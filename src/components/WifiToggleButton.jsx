...
export default function WifiToggleButton({ onClick, connected = false }) {
  const [isChecked, setIsChecked] = useState(connected);
  const [isConnecting, setIsConnecting] = useState(false);

  useEffect(() => {
    setIsChecked(connected);
    if (connected) {
    ...
    <div className="relative w-28 h-28">
      <style jsx>{`
        ...
        .wifi-button.active {
          ...
          box-shadow: 0 8px 32px 0 rgba(134, 239, 172, 0.3);
        }

        .wifi-button.active .icon svg {
          fill: rgb(134, 239, 172);
        }

        .wifi-button.connecting {
          background-color: rgba(77, 124, 255, 0.25);
          border: 3px solid rgba(77, 124, 255, 0.4);
          animation: animeBorder 0.3s linear alternate-reverse infinite;
        }

        .wifi-button.connecting .icon svg {
          fill: rgb(77, 124, 255);
        ...
        @keyframes animeBorder {
          to {
            ...
            box-shadow: 0 8px 32px 0 rgba(77, 124, 255, 0.4);
          }
        }
      `}</style>

      <div 
        className={`wifi-button ${isConnecting ? 'connecting' : ''} ${isChecked && !isConnecting ? 'active' : ''}`}
